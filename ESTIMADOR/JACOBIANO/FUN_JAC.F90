    ...
    real(dp), dimension(nfilas,n), intent(in) :: matriz
	real(dp), dimension(ni*nr*ns,2*ni+1), intent(out) :: l, zz
	...
	integer, intent(in) :: nfilas, ni,nr,ns
	real(dp), dimension(2*ni+1), intent(in) :: v
	real(dp), dimension(ni*nr*ns,2*ni+1), intent(in) :: l,zz
	real(dp), dimension(ni*nr*ns,3*ni) :: Jacobiano
	real(dp), dimension(2*ni+1):: ver
	...
	do j=1,m
		...
		w=w-salto
			...
			!note que si ver==0, en algunas de las dos operaciones sum=0
					  sum1=zz(rayo,ii+1)/(l(rayo,ii+1)*v(ii+1))
			          sum2=zz(rayo,ii+2)/(l(rayo,ii+2)*v(2*ni+2-ii))
					...
					do b=1,salto
						...
						sum1=zz(rayo,ii+1)/(l(rayo,ii+1)*v(ii+1))
					    sum2=zz(rayo,ii+2)/(l(rayo,ii+2)*v(ii+2))
						sum3=zz(rayo,g1)/(l(rayo,g1)*v(2*ni+1-ii))
					    sum4=zz(rayo,g2)/(l(rayo,g2)*v(2*ni+2-ii))	
	...
	!write(1,*),'A='
	!do i=1,n
	!		write(1,*),Jacobiano(i,1:2*ni) !Jacobiano(i,1:m)
	!end do
	!write(1,*),'B='
	!do i=1,n
	!		write(1,*),Jacobiano(i,2*ni+1:m) !Jacobiano(i,1:m)
	!end do
	...
	real(dp), dimension(2*ni+1), intent(in) :: v
	real(dp), dimension(ni*nr*ns,2*ni+1), intent(in) :: l
	integer :: i, j, aux
